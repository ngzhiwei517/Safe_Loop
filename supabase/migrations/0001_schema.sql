...
create table briefings (
  ...
  report_id uuid not null references reports(id) on delete cascade,
  version integer not null,
  body jsonb not null check ((jsonb_typeof(body->'en') = 'string' and btrim(body->>'en') <> '') is true),
...
create table quiz_questions (
  ...
  question jsonb not null check ((jsonb_typeof(question->'en') = 'string' and btrim(question->>'en') <> '') is true),
  explanation jsonb not null check ((jsonb_typeof(explanation->'en') = 'string' and btrim(explanation->>'en') <> '') is true),
  options jsonb not null check (jsonb_options_have_en(options) is true),
