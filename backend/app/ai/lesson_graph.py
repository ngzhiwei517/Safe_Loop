...
class QuizQuestionResult(BaseModel):
    ...
    correct_option: int = Field(ge=0, lt=QUIZ_OPTION_COUNT)
...
async def summarise_case(state: LessonState) -> dict[str, object]:
    ...
    return {"case_summary": _result_points(result.data, sources)}
...
async def translate_zh_cn(state: LessonState) -> dict[str, object]:
    ...
    return {
        ...
        "briefing_zh_cn": _render_sections(
            sections,
            state["procedure_sources"],
            locale="zh-CN",
        ),
